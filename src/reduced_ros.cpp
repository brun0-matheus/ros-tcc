...
ReducedRos::ReducedRos(
    const mpz_class &_p,
    int _n,
    int _extra
): p(_p), n(_n), extra_digits(_extra) {
    compute_pows();
...
vector<int> ReducedRos::decompose_multibase(mpz_class x) const {
    vector<int> digits;
    mpz_class dig;
...
void ReducedRos::set_value(int sess_id, int idx, const mpz_class &value) {
...
vector<mpz_class> ReducedRos::compute_coefficients() {
...
vector<int> ReducedRos::select_challenges(const mpz_class &target) const {
...
int ReducedRos::num_sessions() const { return pows.size(); }

int ReducedRos::num_options(int sess_id) const {
    if(sess_id < 0 || sess_id >= num_sessions())
        _abort("Invalid session id");
    return pows_base[sess_id];
}

/*void ReducedRos::debug_print() {
